...
/// Now supports ItemInstance for proper per-item state management.
/// Supports precise item placement with position and rotation control,
...
public class StorageContainer : BaseInventoryManager
{
    ...
    protected override void OnItemAddedInternal(InventoryItemData item)
    {
        ...
        DebugLog($"Container {containerDisplayName}: Added {item.ItemData?.itemName} at {item.GridPosition} with rotation {item.currentRotation} (Instance: {item.ItemInstance?.InstanceID})");
    ...
    protected override void OnItemRemovedInternal(InventoryItemData item)
    {
        ...
        DebugLog($"Container {containerDisplayName}: Removed {item.ItemData?.itemName}, (Instance: {item.ItemInstance?.InstanceID})");
