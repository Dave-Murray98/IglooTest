...
/// </summary>
public class NPCMoveTowardsPlayer : NPCAction
{
    ...
    {
        if (NPCManager.Instance.PlayerTransform == null)
        {
            Debug.LogWarning($"[MoveTowardsPlayer] No player reference on {gameObject.name}");
            return;
        }

        nPC.movementScript.SetDestination(NPCManager.Instance.PlayerTransform.position);
    }
