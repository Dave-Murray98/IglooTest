...
{
    #region Old Audio System Variables
    ...
    #region New Audio System Variables

    // Add a field to track the current tool loop sound
    private int currentToolLoopSoundID = -1;
    ...
    #region New Audio System

    #region Tool Audio Methods

    public void PlayCurrentToolStartLoopNoise()
    {
        ToolData toolData = EquippedItemManager.Instance.CurrentEquippedItem.GetItemData().ToolData;
        if (toolData != null && toolData.startUseClip != null)
        {
            AudioManager.Instance.PlaySound2D(toolData.startUseClip, AudioCategory.PlayerSFX);
        }
    }

    public void PlayCurrentToolStopLoopNoise()
    {
        ToolData toolData = EquippedItemManager.Instance.CurrentEquippedItem.GetItemData().ToolData;
        if (toolData != null && toolData.stopUseClip != null)
        {
            AudioManager.Instance.PlaySound2D(toolData.stopUseClip, AudioCategory.PlayerSFX);
        }
    }

    public void PlayCurrentToolReloadNoise()
    {
        ToolData toolData = EquippedItemManager.Instance.CurrentEquippedItem.GetItemData().ToolData;

        AudioClip randomReloadClip = toolData.reloadClips[UnityEngine.Random.Range(0, toolData.reloadClips.Length)];

        if (toolData != null && randomReloadClip != null)
        {
            AudioManager.Instance.PlaySound2D(randomReloadClip, AudioCategory.PlayerSFX);
        }
    }

    public void PlayCurrentToolUseLoopNoise()
    {
        // Stop any existing tool loop first
        StopCurrentToolUseLoopNoise();

        ToolData toolData = EquippedItemManager.Instance.CurrentEquippedItem.GetItemData().ToolData;
        if (toolData != null && toolData.useLoopClip != null)
        {
            // Play the looping sound and store its ID
            currentToolLoopSoundID = AudioManager.Instance.PlaySound2DTracked(
                toolData.useLoopClip,
                AudioCategory.PlayerSFX,
                loop: true,
                audioTag: "tool_use"  // Tag it so we can find it by tag too
            );
        }
    }

    public void StopCurrentToolUseLoopNoise()
    {
        // Method 1: Stop by the specific ID we saved
        if (currentToolLoopSoundID != -1)
        {
            AudioManager.Instance.StopLoopingSound(currentToolLoopSoundID, AudioCategory.PlayerSFX);
            currentToolLoopSoundID = -1;
        }

        // Method 2: As a backup, also stop by tag (in case something went wrong with ID tracking)
        // This stops ALL sounds tagged as "tool_use" in the PlayerSFX category
        AudioManager.Instance.StopLoopingSoundsByTag("tool_use", AudioCategory.PlayerSFX);
    }

    #endregion

    #region Ranged Weapon Audio Methods

    #region Consumable Audio Methods

    public void PlayCurrentConsumableStartUseNoise()
    {
        ConsumableData consumableData = EquippedItemManager.Instance.CurrentEquippedItem.GetItemData().ConsumableData;
        if (consumableData != null && consumableData.useAudioClip != null)
        {
            AudioManager.Instance.PlaySound2D(consumableData.useAudioClip, AudioCategory.PlayerSFX);
        }
    }

    #endregion

    // public void PlayCurrentRangedWeaponShootNoise()
    // {
    //     RangedWeaponData weaponData = EquippedItemManager.Instance.CurrentEquippedItem.GetItemData().RangedWeaponData;
    //     if (weaponData != null && weaponData.shootSound != null)
    //     {
    //         AudioManager.Instance.PlaySound2D(weaponData.shootSound, AudioCategory.PlayerSFX);
    //     }
    // }

    public void PlayCurrentRangedWeaponStartReloadNoise()
    {
        RangedWeaponData weaponData = EquippedItemManager.Instance.CurrentEquippedItem.GetItemData().RangedWeaponData;

        if (weaponData != null && weaponData.reloadStartSounds.Length > 0)
        {
            AudioClip randomReloadClip = weaponData.reloadStartSounds[UnityEngine.Random.Range(0, weaponData.reloadStartSounds.Length)];
            AudioManager.Instance.PlaySound2D(randomReloadClip, AudioCategory.PlayerSFX);
        }
    }

    public void PlayCurrentRangedWeaponEndReloadNoise()
    {
        RangedWeaponData weaponData = EquippedItemManager.Instance.CurrentEquippedItem.GetItemData().RangedWeaponData;
        if (weaponData != null && weaponData.reloadEndSounds.Length > 0)
        {
            AudioClip randomReloadClip = weaponData.reloadEndSounds[UnityEngine.Random.Range(0, weaponData.reloadEndSounds.Length)];
