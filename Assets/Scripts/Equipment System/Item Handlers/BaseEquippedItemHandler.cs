...
/// Animation completion handling for proper held action transitions.
///  Added ItemInstance support for proper instance-based state management.
/// </summary>
public abstract class BaseEquippedItemHandler : MonoBehaviour, IEquippedItemHandler
{
    [Header("FX/Projectile Spawn Position Config")]
    [SerializeField] protected Transform fxProjectileSpawnPoint = null;
    ...
    [SerializeField, ReadOnly] protected ItemInstance currentItemInstance;
    [SerializeField, ReadOnly] protected bool hasValidInstance = false;
    ...
    [Header("MeleeHurtBox Handler")]
    [SerializeField] protected PlayerMeleeHurtBoxHandler meleeHurtBoxHandler;
    ...
    public virtual void OnItemEquipped(ItemData itemData)
    {
        ...
        //  Cache ItemInstance reference
        RefreshItemInstanceCache();
        ...
        if (itemData != null && itemData.usesMeleePositionOffset)
            meleeHurtBoxHandler.SetMeleeHurtBoxPosition(itemData.meleeHurtBoxPositionOffset, itemData.useLeftHandHurtBox);

        DebugLog($"Handler activated for {itemData?.itemName ?? "unarmed"} ({HandledItemType})");
    }

    /// <summary>
    /// Override OnItemUnequipped to properly unsubscribe from events
    /// </summary>
    ...
    public virtual bool CanPerformAction(PlayerAnimationType actionType, PlayerStateType playerState)
    {
        ...
        if (!isActive)
        {
            DebugLog($"Cannot perform {actionType} - handler not active");
            return false;
        }
        ...
        {
            DebugLog($"Cannot perform {actionType} - item {currentItemData.itemName} cannot be used in state {playerState}");
            return false;
        }
    ...
    public void UpdateFXProjectileSpawnPointPos(Vector3 spawnPointPos, Vector3 spawnPointRot)
    {
        if (fxProjectileSpawnPoint != null)
        {
            fxProjectileSpawnPoint.transform.localPosition = spawnPointPos;
            fxProjectileSpawnPoint.transform.localRotation = Quaternion.Euler(spawnPointRot);
    ...
    #region  ItemInstance Management

    /// <summary>
    ///  Refresh the cached ItemInstance reference from EquippedItemManager.
    /// Call this when item is equipped or when cache needs updating.
    /// </summary>
    protected virtual void RefreshItemInstanceCache()
    {
        ...
        if (equipmentManager == null)
        {
            DebugLog("Cannot refresh instance cache - EquippedItemManager not found");
            currentItemInstance = null;
            hasValidInstance = false;
            return;
        }

        currentItemInstance = equipmentManager.GetEquippedItemInstance();
        hasValidInstance = currentItemInstance != null;

        if (hasValidInstance)
        {
            DebugLog($"ItemInstance cached: {currentItemInstance.ItemDataName} (ID: {currentItemInstance.InstanceID})");
        }
        else
        {
            DebugLog("No ItemInstance available - handler operating in template-only mode");
        }
    }

    /// <summary>
    ///  Check if we have a valid ItemInstance cached.
    /// Use this for defensive programming before accessing instance data.
    /// </summary>
    protected bool HasValidInstance()
    {
        // If cache is marked invalid, try to refresh once
        if (!hasValidInstance && equipmentManager != null)
        {
            RefreshItemInstanceCache();
        }

        return hasValidInstance && currentItemInstance != null;
    }

    /// <summary>
    ///  Get the ItemInstance, refreshing cache if needed.
    /// Returns null if no valid instance available.
    /// </summary>
    protected ItemInstance GetItemInstance()
    {
        if (!HasValidInstance())
        {
            return null;
    ...
    #region Event-Based Animation System

    /// <summary>
    /// Subscribe to animation completion events from PlayerAnimationManager
    /// </summary>
    ...
    /// <summary>
    /// Unsubscribe from animation completion events
    /// </summary>
    ...
    /// <summary>
    /// Updated animation completion handler that properly handles held action sequences.
    /// </summary>
    ...
    private void HandleEndAnimationCompletion(PlayerAnimationType completedAnimation)
    {
        ...
        CompleteAction();
        HandleHeldActionEndComplete(completedAnimation);
    }
    ...
    protected virtual void HandleEndMelee(bool isLightMeleeAttack)
    {
        ...
        if (currentItemData != null)
            meleeHurtBoxHandler.EnableMeleeHurtBox(currentItemData.useLeftHandHurtBox);
        else // (for unarmed)
            meleeHurtBoxHandler.EnableMeleeHurtBox(false);
    }
    ...
    protected virtual void TriggerAnimation(PlayerAnimationType animationType)
    {
        ...
        try
        {
            // No callback needed - use event system
            ...
            case HeldActionType.Melee:
                {
                    isMeleeing = false;
                    currentMeleeChargeTimer = 0f;
                    meleeHurtBoxHandler.DisableMeleeHurtBoxes();
                    break;
                }
        }
    ...
    protected virtual void OnMeleeReady()
    {
        DebugLog("Melee ready");
    }
    ...
    private void CheckActionTimeout()
    {
        ...
        // Don't timeout during Looping (player likely will hold for a while) or End as the timer carries over to the next action for looing)
        if (currentActionState == ActionState.Looping || currentActionState == ActionState.Ending) return;
    ...
    protected virtual bool CanHandleAction(InputContext context, PlayerAnimationType actionType)
    {
        ...
        {
            DebugLog($"Cannot handle {actionType} action - action not allowed in current state {context.currentPlayerState}");
            return false;
        }
    ...
    protected virtual void OnItemUnequippedInternal()
    {
        ...
        meleeHurtBoxHandler.DisableMeleeHurtBoxes();
        meleeHurtBoxHandler.ResetBothMeleeHurtBoxPositions();
    ...
    protected virtual void Awake()
    {
        ...
        if (meleeHurtBoxHandler == null)
            meleeHurtBoxHandler = GetComponent<PlayerMeleeHurtBoxHandler>();
    }
