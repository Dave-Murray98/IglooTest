...
public class ItemDropValidator : MonoBehaviour
{
    ...
    {
        //removed ground check as game is underwater based, so player needs to be able to drop items anywhere
        ...
        if (HasObstacleAtPosition(position, itemData))
        {
            return new DropValidationResult(false, position, "Obstacle detected at drop position");
        }

        return new DropValidationResult(true, position, "Valid position found");
    }
