...
public class MusicManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Multipliear for music volume (to prevent it from being too loud)
    /// </summary>
    ...
    private void Start()
    {
        AudioManager.Instance.OnVolumeSettingsChanged += OnVolumeSettingsChanged;
        SetupAudioSources();
    ...
    {
        Debug.Log("[MusicManager] Set base volume to " + newBaseVolume + ", with multiplier applied, it will be " + (newBaseVolume * musicVolumeMultiplier));
        newBaseVolume *= musicVolumeMultiplier;
        ...
        if (!isFading)
        {
            if (isHighIntensity)
            {
                if (highIntensitySource != null)
                    highIntensitySource.volume = baseVolume;
                // else
                //     Debug.LogWarning("[MusicManager] High intensity audio source is not assigned!");
            }
            else
            {
                if (lowIntensitySource != null)
                    lowIntensitySource.volume = baseVolume;
                // else
                //     Debug.LogWarning("[MusicManager] Low intensity audio source is not assigned!");
            }
        }
    }
    ...
    {
        SetBaseVolume(AudioManager.Instance.GetMasterVolume() * AudioManager.Instance.GetCategoryVolume(AudioCategory.Music));
    }
