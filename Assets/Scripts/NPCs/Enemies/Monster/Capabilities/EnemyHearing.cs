...
public class EnemyHearing : MonoBehaviour
{
    ...
    private void Start()
    {
        //initialise it as the monster's position to ensure that it won't try to move to an invalid position on startup
        if (lastHeardNoisePosition == Vector3.zero)
            lastHeardNoisePosition = transform.position;
    }
