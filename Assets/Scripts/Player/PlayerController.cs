...
public class PlayerController : MonoBehaviour
{
    ...
    public void RefreshCameraControllerReferences()
    {
    ...
    /// <summary>
    /// Get appropriate speed modifier based on current movement mode, and stamina state
    /// </summary>
    private bool GetSpeedModifierForCurrentMode()
    {
        if (playerStamina != null && playerStamina.IsDepleted)
        {
            return false; // No speed modification if stamina is depleted
        }
    ...
    /// UPDATED: Now applies ADS sensitivity multiplier for consistent aiming
    /// </summary>
    private void HandleHorizontalLookInput(float horizontalInput)
    {
        ...
        // Apply ADS sensitivity multiplier from camera controller
        if (cameraController != null && cameraController.IsADSActive)
        {
            sensitivity *= cameraController.GetADSSensitivityMultiplier();
        }
    ...
    private bool CanStartClimbing()
    {
        ...
        {
            DebugLog("IsStateTransitioning, returning false");
            return false;
        }
        ...
        {
            DebugLog("either is currently in vehicle or already climbing: returning false");
            return false;
        }
        ...
        {
            DebugLog("ledge detector is either null or doesn't have ledge, returning false");
            return false;
        }
