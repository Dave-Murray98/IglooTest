...
public class NPCPathfindingUtilities : MonoBehaviour
{
    ...
    {
        DebugLog($"GetRandomValidPosition at agentPos: {agentPos}");
        ...
        //get the world transform point (for some reason the volume's Bounds are always local to the volume)
        Bounds volumeBoundsWorldTransformed = new Bounds(
            closestVolume.transform.TransformPoint(volumeBounds.center),
            Vector3.Scale(volumeBounds.size, closestVolume.transform.lossyScale)
        );

        DebugLog($"GetRandomValidPosition using volume bounds: {volumeBounds},  in world space:  {volumeBoundsWorldTransformed}");

        // Generate random positions within the volume bounds and test them
        Vector3 validPosition = GenerateRandomValidPositionInBounds(volumeBoundsWorldTransformed, NavAreaTypes.Volume);
        ...
        if (validPosition == Vector3.zero)
        {
            ...
            validPosition = closestVolume.transform.TransformPoint(closestVolume.Data.Regions[randomIndex].Bounds.center);
        }
    ...
    public Vector3 GetRandomValidPositionNearPoint(Vector3 point, float radius = 20f)
    {
        ...
        {
            DebugLogWarning($"GetRandomValidPositionNearPoint called for Vector3.zero");
        }
        ...
        if (validPosition == Vector3.zero)
        {
            ...
            if (closestVolume == null)
            {
                DebugLogWarning("closestVolume is null");
    ...
    private Vector3[] GenerateRandomPointsInSphere(Vector3 center, float radius, int count)
    {
        ...
        DebugLog("============ GenerateRandomPointsInSphere ===========");

        DebugLog($"Sphere center = {center}. Radius = {radius}");
        foreach (Vector3 pos in points)
        {
            DebugLog($"random point.position = {pos}");
        }
    ...
    private Vector3[] GenerateRandomPointsInBounds(Bounds bounds, int count)
    {
        ...
        //log the bounds' world position (not it's local position)
    ...
    private Vector3 FindValidPositionFromCandidates(Vector3[] candidates, NavAreaTypes areaTypeMask)
    {
        ...
        {
            DebugLogWarning($"candidates is either null or has no entries");
            return Vector3.zero;
        }
        ...
        DebugLog("==================== NavSampleResults in FindValidPositionFromCandidates: ====================");
    ...
    {
        DebugLog($"GetRandomValidPositionInRadius called for {centerPosition}");
    ...
    {
        DebugLog($"GetNearestValidPosition at targetPos: {targetPosition}");
        ...
        DebugLogWarning($"NavSampleJob.SamplePositionInAllAreas({targetPosition} failed, trying positions in an expanding radius)");
