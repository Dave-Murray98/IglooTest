...
/// UPDATED: Now works with ItemInstance for proper state management.
/// </summary>
public abstract class BaseInventoryDragHandler : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
{
    ...
    [Header("ItemType Display Components")]
    [SerializeField] protected TextMeshProUGUI itemQuantityDisplayText;
    ...
    protected bool isProcessingDropdownAction = false;
    ...
    public virtual void Initialize(InventoryItemData item, BaseInventoryGridVisual visual)
    {
        ...
        DebugLog($"Initialized drag handler for item: {inventoryItemData?.ItemData?.itemName}");
    }

    /// <summary>
    /// Update item type specific display elements.
    /// </summary>
    private void UpdateItemQuantityDisplay()
    {
        ...
        switch (inventoryItemData.ItemData.itemType)
        {
            case ItemType.RangedWeapon:
                displayText = $"{inventoryItemData.ItemInstance.RangedWeaponInstanceData.currentAmmoInClip} / {inventoryItemData.ItemData.RangedWeaponData.maxAmmo}";
                break;

            case ItemType.Bow:
                displayText = inventoryItemData.ItemInstance.BowInstanceData.hasArrowNocked ? "1" : "0";
                break;
            case ItemType.Ammo:
                displayText = inventoryItemData.ItemInstance.AmmoInstanceData.currentAmmo.ToString();
                break;
            case ItemType.OxygenTank:
                int currentOxygenInt = Mathf.RoundToInt(inventoryItemData.ItemInstance.OxygenTankInstanceData.currentOxygen);
                displayText = itemQuantityDisplayText.text = $"{currentOxygenInt} / {inventoryItemData.ItemData.OxygenTankData.maxCapacity}";
                break;
            case ItemType.Tool:
                if (inventoryItemData.ItemInstance.ItemData.ToolData.requiresEnergySource)
                    displayText = $"{inventoryItemData.ItemInstance.ToolInstanceData.equippedEnergySourceAmount} / {inventoryItemData.ItemData.ToolData.maxEnergyCapacity}";
                break;
            case ItemType.ToolEnergySource:
                displayText = $"{inventoryItemData.ItemInstance.ToolEnergySourceInstanceData.currentEnergy} / {inventoryItemData.ItemData.ToolData.maxEnergyCapacity}";
                break;
        }

        //for other types, just leave blank

        if (itemQuantityDisplayText != null)
        {
            itemQuantityDisplayText.text = displayText;
        }
    ...
    {
        if (rectTransform != null && inventoryItemData != null && gridVisual != null)
        {
            Vector2 gridPos = gridVisual.GetCellWorldPosition(inventoryItemData.GridPosition.x, inventoryItemData.GridPosition.y);
    ...
    {
        if (isDragging && canRotate && inventoryItemData?.CanRotate == true)
        {
    ...
    protected virtual void ShowDropdownMenu(Vector2 screenPosition)
    {
        ...
        dropdownMenu.ShowMenu(inventoryItemData, screenPosition, inventoryTypeContext, canTransfer);
    }
    ...
    public virtual void OnBeginDrag(PointerEventData eventData)
    {
        ...
        if (!canDrag || inventoryItemData == null) return;
    ...
    protected virtual void BeginDragInternal(PointerEventData eventData)
    {
        ...
        originalGridPosition = inventoryItemData.GridPosition;
        originalRotation = inventoryItemData.currentRotation;
        itemRemovedFromGrid = false;

        TriggerStatsDisplay();

        // Remove item from grid to prevent self-collision during drag
        if (gridVisual.GridData.GetItem(inventoryItemData.ID) != null)
        {
        ...
        DebugLog($"Began dragging item: {inventoryItemData.ItemData?.itemName}");
    }
    ...
    public virtual void OnDrag(PointerEventData eventData)
    {
        ...
        if (!isDragging || inventoryItemData == null || gridVisual == null) return;
    ...
    protected virtual void ShowInventoryPreview()
    {
        ...
        var tempItem = new InventoryItemData(inventoryItemData.ID + "_temp", inventoryItemData.ItemData, gridPos);
        tempItem.SetRotation(inventoryItemData.currentRotation);
    ...
    public virtual void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (!isDragging || inventoryItemData == null) return;
    ...
    protected virtual void EndDragInternal(PointerEventData eventData)
    {
        ...
        DebugLog($"Ended dragging item: {inventoryItemData.ItemData?.itemName}");
    }
    ...
    protected virtual void HandleInventoryDrop()
    {
        ...
        if (IsWithinGridBounds() && wasValidPlacement)
        {
            ...
            inventoryItemData.SetGridPosition(targetGridPos);
    ...
    protected virtual void RotateItemDuringDrag()
    {
        ...
        var currentRotation = inventoryItemData.currentRotation;
        var currentCenter = GetVisualCenter();

        int maxRotations = TetrominoDefinitions.GetRotationCount(inventoryItemData.shapeType);
        int newRotation = (currentRotation + 1) % maxRotations;

        inventoryItemData.SetRotation(newRotation);
    ...
    {
        var shapeData = inventoryItemData.CurrentShapeData;
    ...
    {
        return canDrag && inventoryItemData != null;
    }
    ...
    protected virtual void OnDropdownActionSelected(InventoryItemData selectedItem, string actionId)
    {
        ...
        // CRITICAL: Only handle actions for OUR item
        if (inventoryItemData != selectedItem)
        {
            DebugLog($"Ignoring dropdown action for different item (Our ID: {inventoryItemData?.ID}, Selected ID: {selectedItem?.ID})");
            return;
        }

        // SAFEGUARD: Check if we're already processing an action
        if (isProcessingDropdownAction)
        {
            DebugLogWarning($"Already processing dropdown action - ignoring duplicate call for {actionId}");
            return;
        }

        isProcessingDropdownAction = true;
        ...
        finally
        {
            ...
            isProcessingDropdownAction = false;
        }
    }
    #endregion
    ...
    {
        Vector2 targetPos = gridVisual.GetCellWorldPosition(inventoryItemData.GridPosition.x, inventoryItemData.GridPosition.y);
