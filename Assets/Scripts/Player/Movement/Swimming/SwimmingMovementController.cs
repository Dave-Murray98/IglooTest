...
public class SwimmingMovementController : MonoBehaviour, IMovementController
{
    ...
    [SerializeField] private float forwardMovementMultiplier = 1f;
    [SerializeField] private float strafeMovementMultiplier = 0.5f;
    [SerializeField] private float backwardMovementMultiplier = 0.5f;
    ...
    /// <summary>
    /// Apply camera-relative swimming movement with directional speed multipliers
    /// </summary>
    private void ApplySwimmingMovement()
    {
        ...
        if (swimDirection.magnitude > 0.01f)
        {
            ...
            // Calculate directional multiplier based on movement input
            float directionalMultiplier = CalculateDirectionalMultiplier();

            // Apply the multiplier to the speed
            currentSpeed *= directionalMultiplier;
    ...
    /// <summary>
    /// Calculate movement speed multiplier based on whether player is moving forward, backward, or strafing
    /// </summary>
    private float CalculateDirectionalMultiplier()
    {
        // If there's no input, return 1 (no multiplier)
        if (movementInput.magnitude < 0.01f)
            return 1f;

        // Get the dominant input direction
        float forwardInput = movementInput.y;  // Positive = forward, Negative = backward
        float strafeInput = Mathf.Abs(movementInput.x);  // How much side-to-side movement

        // Determine which multiplier to use based on input
        // Check forward movement first
        if (forwardInput > 0.1f)
        {
            // Moving forward - blend between forward and strafe multipliers
            float strafeInfluence = strafeInput;

            // If mostly forward, use forward multiplier
            // If diagonal (forward + strafe), blend the two
            return Mathf.Lerp(
                forwardMovementMultiplier,
                (forwardMovementMultiplier + strafeMovementMultiplier) * 0.5f,
                strafeInfluence
            );
        }
        ...
        {
            // Moving backward - blend between backward and strafe multipliers
            float strafeInfluence = strafeInput;

            return Mathf.Lerp(
                backwardMovementMultiplier,
                (backwardMovementMultiplier + strafeMovementMultiplier) * 0.5f,
                strafeInfluence
            );
        }
        // Pure strafing (left/right only)
        else
        {
            return strafeMovementMultiplier;
        }
    }
