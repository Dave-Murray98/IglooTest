...
public class StorageContainerGridVisual : BaseInventoryGridVisual
{
    ...
    // StorageContainerGridVisual.cs - UPDATED TRANSFER METHODS
    // Add these updated methods to the existing class

    #region Transfer Operations

    /// <summary>
    /// REFACTORED: Transfer an item from this container to the player inventory.
    /// Now uses InventoryItemData which contains ItemInstance for proper state preservation.
    /// </summary>
    public bool TransferItemToPlayer(string itemId)
    {
        ...
        // Get the InventoryItemData (which contains ItemInstance)
        ...
        DebugLog($"Found item {item.ItemData?.itemName} in container (Instance: {item.ItemInstance?.InstanceID})");
        ...
        if (containerManager.RemoveItem(itemId))
        {
            ...
            if (PlayerInventoryManager.Instance.AddItem(item))
            {
            ...
            else
            {
                ...
                containerManager.AddItem(item, item.GridPosition, item.currentRotation);
                return false;
            }
        }
    ...
    /// <summary>
    /// REFACTORED: Transfer an item from the player inventory to this container.
    /// Now uses InventoryItemData for proper ItemInstance state preservation.
    /// </summary>
    public bool TransferItemFromPlayer(string playerItemId)
    {
        ...
        // Get the InventoryItemData from player inventory (contains ItemInstance)
        ...
        DebugLog($"Found item {item.ItemData?.itemName} in player inventory (Instance: {item.ItemInstance?.InstanceID})");
        ...
        if (PlayerInventoryManager.Instance.RemoveItem(playerItemId))
        {
            // REFACTORED: Pass InventoryItemData to AddItem
            // BaseInventoryManager.AddItem(InventoryItemData) will create a NEW ItemInstance copy
            if (containerManager.AddItem(item))
            {
            ...
            else
            {
                ...
                DebugLogWarning("Failed to add item to container - restoring to player inventory");

                // Restore using InventoryItemData (preserves ItemInstance state)
                PlayerInventoryManager.Instance.AddItem(item, item.GridPosition, item.currentRotation);
                return false;
            }
        }
