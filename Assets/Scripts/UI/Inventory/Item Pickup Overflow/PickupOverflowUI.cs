...
public class PickupOverflowUI : MonoBehaviour
{
    ...
    // Events
    public System.Action<ItemInstance> OnPickupOverflowOpened;
    ...
    /// </summary>
    private void GetPickupManager()
    {
        pickupManager = GetComponent<PickupOverflowManager>();
    ...
    private void ConnectToInputManager()
    {
        ...
        {
            InputManager.Instance.OnUIInteractPressed -= HandleInteractInput;
            InputManager.Instance.OnUIInteractPressed += HandleInteractInput;
    ...
    /// <summary>
    /// REFACTORED: Show the pickup overflow UI for an item from ItemData.
    /// Creates new ItemInstance.
    /// </summary>
    public void ShowPickupOverflow(ItemInstance itemInstance)
    {
        if (itemInstance == null)
        {
            DebugLogError("Cannot show pickup overflow for null item instance");
            return;
        }
        ...
        // Set the pickup item (creates new ItemInstance)
        if (pickupManager.SetPickupItem(itemInstance))
        {
            ...
            DebugLog($"Opened pickup overflow UI for: {itemInstance.ItemData.itemName} at time {openTime:F2}");
            OnPickupOverflowOpened?.Invoke(itemInstance);
        }
        else
        {
            DebugLogError($"Failed to set pickup item: {itemInstance.ItemData.itemName}");
        }
    }

    /// <summary>
    /// REFACTORED: Show the pickup overflow UI for an existing item with state.
    /// Preserves ItemInstance state from world pickup.
    /// </summary>
    public void ShowPickupOverflow(InventoryItemData existingItem)
    {
        ...
        // Set the pickup item (preserves ItemInstance state)
        if (pickupManager.SetPickupItem(existingItem))
        {
            pickupItemWorldPosition = playerController.transform.position;
            isOpen = true;
            openTime = Time.time;
            ...
            DebugLog($"Opened pickup overflow UI for: {existingItem.ItemData.itemName} (Instance: {existingItem.ItemInstance?.InstanceID}) at time {openTime:F2}");
            OnPickupOverflowOpened?.Invoke(existingItem.ItemInstance);
        }
        else
        {
            DebugLogError($"Failed to set pickup item: {existingItem.ItemData.itemName}");
        }
    }
    ...
    private void ShowUI()
    {
        ...
        {
            AudioManager.Instance.PlaySound2D(GameManager.Instance.uiManager.openMenuSound, AudioCategory.UI);
    ...
    private void HideUI()
    {
        ...
        if (!GameManager.Instance.uiManager.ShouldToggleMenuModeInput())
        {
            GameManager.Instance.uiManager.ToggleUIMenuModeInput(false);
            AudioManager.Instance.PlaySound2D(GameManager.Instance.uiManager.closeMenuSound, AudioCategory.UI);
    ...
    private void DropPickupItem()
    {
        ...
        //commented out to hide errors in the itemdropsystem
