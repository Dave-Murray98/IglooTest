...
public class ADSController : MonoBehaviour
{
    ...
    /// <summary>
    /// Subscribe to input manager events for hold-to-ADS
    /// </summary>
    private void SubscribeToInputEvents()
    {
        if (InputManager.Instance != null)
        {
            InputManager.Instance.OnADSHeldStart += OnADSInputPressed;
            InputManager.Instance.OnADSReleased += OnADSInputReleased;
            DebugLog("Subscribed to ADS input events for hold-to-ADS");
        }
        else
        {
            Debug.LogError("[ADSController] InputManager not found! ADS input will not work.");
        }
    }
    ...
    public void StartAimingDownSights()
    {
        ...
        if (currentWeapon.itemType != ItemType.RangedWeapon && currentWeapon.itemType != ItemType.Bow)
        {
            return;
        }

        // Allow starting even if transitioning OUT of ADS
        if (adsPosition == null)
            return;

        // If already aiming and not transitioning, don't restart
    ...
    {
        // Allow stopping even if transitioning INTO ADS
        var cameraRoot = cameraController?.CameraRoot;
        if (cameraRoot == null) return;

        // If already not aiming and not transitioning, don't do anything
        if (!isAimingDownSights && !isTransitioningADS)
            return;
    ...
    /// <summary>
    /// Called when ADS button is pressed down
    /// </summary>
    private void OnADSInputPressed()
    {
        // Only start ADS if we're not already aiming
        if (!isAimingDownSights)
        {
            StartAimingDownSights();
        }
    }

    /// <summary>
    /// Called when ADS button is released
    /// </summary>
    private void OnADSInputReleased()
    {
        // Stop ADS as soon as button is released, even during transition
        if (isAimingDownSights)
        {
    ...
    public void Cleanup()
    {
        ...
        // NEW: Unsubscribe from input events
        if (InputManager.Instance != null)
        {
            InputManager.Instance.OnADSHeldStart -= OnADSInputPressed;
            InputManager.Instance.OnADSReleased -= OnADSInputReleased;
