...
/// <summary>
/// REFACTORED: Utility class for clothing and inventory integration with ItemInstance support.
/// Provides centralized validation, error handling, and user feedback.
/// Now properly works with ItemInstance for accurate state validation.
/// </summary>
public static class ClothingInventoryUtilities
{
    ...
    /// Works with ItemInstance for proper validation.
    /// </summary>
    public static ValidationResult ValidateSwapOperation(InventoryItemData newItem, ClothingLayer targetLayer)
    {
        ...
        // REFACTORED: Get the ItemInstance from the equipped item
        var currentItemInstance = slot.GetEquippedItemInstance();
        if (currentItemInstance?.ItemData == null)
        {
            return new ValidationResult(false, "Current equipped item instance not found");
        }

        // Validate same-space swap: can the displaced item fit where the new item currently is?
        var sameSpaceValidation = ValidateSameSpaceSwap(newItem, currentItemInstance.ItemData);
    ...
    /// <summary>
    /// Validates that displaced item can fit in the same space as the new item.
    /// This is the core logic for same-space swapping.
    /// </summary>
    public static ValidationResult ValidateSameSpaceSwap(InventoryItemData newItem, ItemData displacedItemData)
    {
        ...
        // Create a temporary ItemInstance for the displaced clothing at the original position
        var tempDisplacedInstance = new ItemInstance(displacedItemData);
        var tempDisplacedItem = new InventoryItemData("temp_displaced_validation", tempDisplacedInstance, originalPosition);
