...
public class PlayerSaveComponent : SaveComponentBase, IPlayerDependentSaveable
{
    ...
    /// <summary>
    /// Finds all required player component references
    /// </summary>
    ...
    /// <summary>
    /// Validates all component references are properly set
    /// </summary>
    private void ValidateReferences()
    {
        ...
        if (playerManager == null)
            Debug.LogWarning($"[{name}] PlayerManager reference missing! Some functionality may be limited.");
    ...
    /// <summary>
    /// Collects player data for saving - HEALTH REMOVED, handled by PlayerHealthSaveComponent
    ...
    public override object GetDataToSave()
    {
        ...
        // REMOVED: Health extraction - now handled by PlayerHealthSaveComponent
        // Health will be managed separately and combined in the unified save system
        ...
        DebugLog($"Save data created: {saveData.GetMovementDebugInfo()} - Health handled separately");
        return saveData;
    }
    ...
    /// <summary>
    /// Extracts relevant player data from save containers - HEALTH REMOVED
    /// </summary>
    public override object ExtractRelevantData(object saveContainer)
    {
        DebugLog("Extracting player save data for persistence (health handled separately)");
        ...
        else if (saveContainer is PlayerPersistentData persistentData)
        {
            ...
            {
                // REMOVED: Health extraction - PlayerHealthSaveComponent handles this
    ...
    /// <summary>
    /// Extracts player data from unified save - HEALTH REMOVED
    /// </summary>
    public object ExtractFromUnifiedSave(PlayerPersistentData unifiedData)
    {
        if (unifiedData == null) return null;

        DebugLog("Using modular extraction from unified save data (health handled separately)");

        var playerSaveData = new PlayerSaveData
        {
            // REMOVED: Health fields - handled by PlayerHealthSaveComponent
    ...
    /// <summary>
    /// Creates default player data for new games - HEALTH REMOVED
    /// </summary>
    public object CreateDefaultData()
    {
        DebugLog("Creating default player data for new game (health handled separately)");

        var defaultData = new PlayerSaveData();

        // REMOVED: Health initialization - PlayerHealthSaveComponent handles this

        // Set settings from PlayerData if available
        if (playerData != null)
        {
            defaultData.maxHealth = playerData.maxHealth; // Keep for compatibility, but health component manages actual health
            defaultData.lookSensitivity = playerData.lookSensitivity;
        }
        else
        {
            defaultData.maxHealth = 100f; // Compatibility fallback
    ...
    /// <summary>
    /// Contributes player data to unified save - HEALTH REMOVED
    /// </summary>
    public void ContributeToUnifiedSave(object componentData, PlayerPersistentData unifiedData)
    {
        ...
        {
            DebugLog("Contributing player data to unified save structure (health handled separately)");

            // Store basic stats in main structure - REMOVED HEALTH
            // Health is now handled by PlayerHealthSaveComponent
    ...
    /// <summary>
    /// Context-aware data restoration with vehicle coordination - HEALTH REMOVED
    /// </summary>
    public override void LoadSaveDataWithContext(object data, RestoreContext context)
    {
        ...
        DebugLog($"=== PLAYER DATA RESTORATION (Context: {context}) - Health handled separately ===");
        ...
        // STEP 1: Restore abilities (REMOVED health restoration)
        RestorePlayerAbilities(playerSaveData);
    ...
    /// <summary>
    /// Sets initial movement mode using PlayerController
    /// </summary>
    ...
    /// <summary>
    /// Schedules movement validation using PlayerStateManager
    /// </summary>
    ...
    /// <summary>
    /// Restores player abilities - HEALTH REMOVED, now only abilities and settings
    /// </summary>
    private void RestorePlayerAbilities(PlayerSaveData playerSaveData)
    {
        ...
        // REMOVED: Health restoration - now handled by PlayerHealthSaveComponent
        // PlayerHealthSaveComponent will handle health restoration separately
    ...
    {
        DebugLog("Preparing player data for save (health handled separately)");
    ...
    {
        DebugLog("Player data load completed (health handled separately)");
