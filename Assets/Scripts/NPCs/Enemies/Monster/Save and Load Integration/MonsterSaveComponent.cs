...
/// - Movement system is always active and responds to target changes automatically
/// </summary>
public class MonsterSaveComponent : SaveComponentBase
{
    ...
    /// SIMPLIFIED: No need to manually deactivate movement
    /// </summary>
    private void PrepareMonsterForRestoration()
    {
        DebugLog("Preparing monster for restoration");

        // SIMPLIFIED: Just clear the target to pause movement
        // No need to call DeactivateMovement()
        if (monsterController != null)
        {
            monsterController.SetTargetPosition(Vector3.zero);
        }
    ...
    /// SIMPLIFIED: No need to manually activate movement
    /// </summary>
    private void CompleteMonsterRestoration()
    {
        DebugLog("Completing monster restoration");

        // SIMPLIFIED: Movement will resume automatically when the state machine
        // sets a new target position. No need to call ActivateMovement()

        // The state machine will handle setting appropriate destinations
        // based on the restored state

        DebugLog("Monster restoration complete - movement will resume with state machine");
    }
