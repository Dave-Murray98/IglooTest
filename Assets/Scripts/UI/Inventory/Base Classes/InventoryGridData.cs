...
public class InventoryGridData
{
    ...
    {
        DebugLog($"GetItem: Looking for item {itemID}");

        if (items.ContainsKey(itemID))
        {
            DebugLog($"GetItem: Found item {itemID}");
            return items[itemID];
        }

        DebugLog($"GetItem: Item {itemID} not found");
        return null;
    }
    ...
    /// <summary>
    /// Returns a bool if the inventory has at least one of the specified items in it.
    /// </summary>
    public virtual bool HasItem(ItemData itemData)
    {
        return items.Values.Any(i => i.ItemData == itemData);
    }
