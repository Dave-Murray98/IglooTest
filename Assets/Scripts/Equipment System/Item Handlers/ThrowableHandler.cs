...
public class ThrowableHandler : BaseEquippedItemHandler
{
    ...
    protected override void OnItemEquippedInternal(ItemData itemData)
    {
        ...
        UpdateFXProjectileSpawnPointPos(itemData.ThrowableData.throwableSpawnPoint, itemData.ThrowableData.throwableSpawnRotation);
    ...
    protected override void ExecuteSecondaryAction(PlayerAnimationType actionType)
    {
        ...
        //ThrowItem(); - this is now called via an animation event in the throwable's animation
    }
    ...
    /// <summary>
    /// Execute the throw
    /// This is called via an animation event in the throwable's animation
    /// </summary>
    public void ThrowItem()
    {
        ...
        StartCoroutine(SpawnThrowableAfterIK());
    ...
    /// <summary>
    /// Spawn Throwable after waiting for Final IK to complete (as the bullet spawn pos will have been updated by the IK)
    /// </summary>
    private System.Collections.IEnumerator SpawnThrowableAfterIK()
    {
    ...
    /// <summary>
    /// Spawn Throwable from bullet pool and apply force
    /// </summary>
    private void SpawnThrowable()
    {
        if (CurrentItemData == null || CurrentItemData.ThrowableData == null)
        {
            Debug.LogWarning("[ThrowableHandler] Cannot spawn throwable - missing throwable data");
            return;
        }

        if (PlayerThrowablePool.Instance == null)
        {
            Debug.LogError("[ThrowableHandler] PlayerThrowablePool.Instance is null!");
            return;
        }

        if (fxProjectileSpawnPoint == null)
        {
            Debug.LogWarning("[ThrowableHandler] No throwable spawn point assigned!");
            return;
        }

        // Cache spawn transform
        Vector3 spawnPosition = equipmentManager.visualManager.currentActiveItem.transform.position;
        Quaternion spawnRotation = equipmentManager.visualManager.currentActiveItem.transform.rotation;

        DebugLog($"Requesting throwable spawn at Position: {spawnPosition}, Rotation: {spawnRotation.eulerAngles}");

        // Get projectile from pool (already positioned)
        PlayerThrowable throwable = PlayerThrowablePool.Instance.GetThrowable(
            CurrentItemData,
            spawnPosition,
            spawnRotation
        );
        ...
        // Initialize throwable (activates it)
        throwable.Initialize(damage, currentItemData, spawnPosition, spawnRotation);


        // Remove item from inventory (throwables are consumed) We're doing this 
        // here because we need to wait for the throwable to be spawned before we can remove it
        RemoveItemFromInventory();
    }

    /// <summary>Get current weapon damage (from template)</summary>
    public float GetDamage() => ThrowableData?.damage ?? 10f;
