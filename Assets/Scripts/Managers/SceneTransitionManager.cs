...
public class SceneTransitionManager : MonoBehaviour
{
    ...
    /// UPDATED: Coordinator is now scene-based, handled by SceneDataManager
    /// </summary>
    private System.Collections.IEnumerator HandleSaveFileLoadRestore(string sceneName)
    {
        ...
        // 2. Restore scene data from save file (this now includes coordinator state)
    ...
    private void PositionPlayerAtDoorway(string doorwayID)
    {
        ...
        DoorwayToOtherLevel[] doorways = FindObjectsByType<DoorwayToOtherLevel>(FindObjectsSortMode.None);
        DoorwayToOtherLevel targetDoorway = System.Array.Find(doorways, d => d.doorwayID == doorwayID);
    ...
    private DialogueSaveData ExtractDialogueSaveData(Dictionary<string, object> saveData)
    {
        var dialogueData = new DialogueSaveData();

        if (saveData.ContainsKey("HandlerSpeechManager"))
            dialogueData.handlerSpeechData = saveData["HandlerSpeechManager"] as HandlerSpeechSaveData;

        if (saveData.ContainsKey("AudioLogManager"))
            dialogueData.audioLogData = saveData["AudioLogManager"] as AudioLogManagerSaveData;
